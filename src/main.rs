...
use executor::{Executor, WrapperFn};
use std::sync::Arc;
use std::any::Any;
...
async fn main() {
    ...
    // 注册任务并应用包装函数
    executor.register_task(my_task().with_wrapper(&param_wrapper).with_wrapper(&result_wrapper));
    executor.register_task(test().with_wrapper(&param_wrapper).with_wrapper(&result_wrapper));
...
#[cte_task(name = "test")]
async fn test(ctx: executor::Context, payload: Value) -> Result<String, String>{
    Ok("test".to_string())
}
// 使用宏定义任务
#[cte_task(name = "my_task")]
async fn my_task(ctx: executor::Context, payload: Value) -> Result<String, String> {
